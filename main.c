 ...
 *  this checks if a candidafe soltuion is at a given co-ordinate
...
int main(int argc, const char * argv[])
{
    ...
    while(currentListOfCandidates.indexOfLastEntryAdded >= 0) {
        if (workingCandidate.variableValues[XCOORD] == ENDX && workingCandidate.variableValues[YCOORD] == ENDY) {
            break;
        }
        ...
        int index_of_best = 0;
        int best_score = currentListOfCandidates.listEntries[currentListOfCandidates.indexOfLastEntryAdded].score;
        
        for(int n = 0; n <= currentListOfCandidates.indexOfLastEntryAdded; n++) {
            
            if(currentListOfCandidates.listEntries[n].score < best_score) {
                best_score = currentListOfCandidates.listEntries[n].score;
                index_of_best = n;
            }
        }
            AddWorkingCandidateToExaminedList();
            CopySolutionFromCurrentListIntoWorkingCandidate(index_of_best);
            RemoveSolutionFromCurrentList(index_of_best);
        }
